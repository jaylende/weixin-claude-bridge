...
/** 解密后媒体文件的本地路径（原 inbound.ts 中的类型，随本模块内联）。 */
...
export async function downloadMediaFromItem(
  ...
  if (item.type === MessageItemType.IMAGE) {
    ...
    try {
      ...
      logger.debug(`${label} voice: decrypted ${silkBuf.length} bytes (raw silk, 无转码依赖)`);
      ...
      logger.debug(`${label} voice: saved raw SILK to ${saved.path}`);
    } catch (err) {
