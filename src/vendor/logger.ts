...
/**
 * Bridge logger — writes JSON lines to the bridge state dir:
 *   <stateDir>/logs/bridge-YYYY-MM-DD.log
 * Same file and format used by the upstream openclaw-weixin channel.
 */

const MAIN_LOG_DIR = path.join(
  process.env.BRIDGE_STATE_DIR || path.join(os.tmpdir(), "weixin-claude-bridge"),
  "logs",
);
const SUBSYSTEM = "weixin-claude-bridge";
const RUNTIME = "node";
const RUNTIME_VERSION = process.versions.node;
const HOSTNAME = os.hostname() || "unknown";
const PARENT_NAMES = ["weixin-claude-bridge"];
...
function resolveMainLogPath(): string {
  ...
  return path.join(MAIN_LOG_DIR, `bridge-${dateKey}.log`);
}
