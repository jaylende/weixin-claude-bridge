...
/** Self-declared bot_agent (UA-style) — observability only, not used for auth/routing. */
const BRIDGE_BOT_AGENT = "ClaudeWeixinBridge/1.0.0";

/** Build the `base_info` payload included in every API request. */
export function buildBaseInfo(): BaseInfo {
  return {
    channel_version: CHANNEL_VERSION,
    bot_agent: sanitizeBotAgent(BRIDGE_BOT_AGENT),
  };
}
