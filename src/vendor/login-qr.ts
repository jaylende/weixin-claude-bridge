...
/** 获取本地已保存的 bot token 列表（bridge 为单账号，最多 1 个）。 */
function getLocalBotTokenList(): string[] {
  const stateDir = process.env.BRIDGE_STATE_DIR || "state";
  const botPath = path.join(stateDir, "bot.json");
  try {
    const parsed = JSON.parse(fs.readFileSync(botPath, "utf-8")) as { token?: string };
